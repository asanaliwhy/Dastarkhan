...
import { readPantry, shoppingQuantity, csvCell, type Pantry } from './commerce';
...
export function PlanViews({ view, days, config, products, day, setDay, generate, setRecipe, setView, priceStatus }: Props) {
  ...
  const rawCart = useMemo(() => groceries(days, products, config.stores), [days, products, config.stores]);
  const cart = rawCart.map(item=>{const {needed,packs}=shoppingQuantity(item.amount,item.product.packAmount,pantry[item.product.id]);return {...item,needed,packs,cost:packs*item.price};});
  const checkKey='dastarkhan.checked.v2:'+JSON.stringify(days.map(d=>d.meals.map(m=>[m.recipe.id,m.scale])));
  const checkedCount=cart.filter(item=>checked.includes(item.product.id)).length;
  function updatePantry(value:Pantry) {setPantry(value);try{localStorage.setItem('dastarkhan.pantry.v1',JSON.stringify(value));}catch{setStorageNotice('Device storage is full or unavailable. Changes will last until you close this page.');}}
  useEffect(()=>{try{setPantry(readPantry(JSON.parse(localStorage.getItem('dastarkhan.pantry.v1')??'{}')));}catch{}},[]);
  const stores = useMemo(() => [...new Set(cart.map((item) => item.store))], [cart]);
  const visibleCart = storeFilter === "all" || !stores.includes(storeFilter) ? cart : cart.filter((item) => item.store === storeFilter);
  const total = cart.reduce((sum, item) => sum + item.cost, 0);
  const targets = macroTargets(config);

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(checkKey) || "[]") as unknown;
  ...
  function toggleChecked(id: string) {
    saveChecks(checked.includes(id) ? checked.filter(item=>item!==id) : [...checked,id]);
  }

  function saveChecks(value:string[]) {setChecked(value);try{localStorage.setItem(checkKey,JSON.stringify(value));}catch{setStorageNotice('Could not save checkmarks on this device.');}}
  function download() {
    const rows = [
  ...
  function markAll() { saveChecks(checkedCount === cart.length ? [] : cart.map(item=>item.product.id)); }
  ...
  return <>
    ...
    <div className="plan-stats">
      <div><small>Weekly basket</small><b>{money(total)}</b><span>{total > config.max ? "Above your maximum budget" : total < config.min ? "Below your preferred range" : "Within your maximum budget"}</span></div>
    ...
    {view === 1 ? <>
      <div className="toolbar"><div className="days" role="group" aria-label="Choose a day">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((name, index) => <button key={name} aria-pressed={day === index} className={day === index ? "active" : ""} onClick={() => setDay(index)}>{name}</button>)}</div><button className="secondary" onClick={() => generate(day + 1)}><RefreshCw size={15} /> New combination</button></div>
      ...
      <div className="macro-target-note">Daily target: {targets.kcal} kcal · {targets.protein} g protein · {targets.carbs} g carbs · {targets.fat} g fat. Values use generic nutrition estimates; package labels are not verified.</div>
      ...
      <PantryPanel products={products} pantry={pantry} update={updatePantry}/>{storageNotice&&<p role="status">{storageNotice}</p>}
      <div className="split-heading"><div><h2>Your grocery list</h2><p>{checkedCount} of {cart.length} items checked · {stores.length} stores</p></div><div className="grocery-actions"><button className="secondary" onClick={markAll}><CheckCheck size={15} /> {checkedCount === cart.length ? "Clear checks" : "Mark all"}</button><button className="secondary" onClick={download}><Download size={15} /><span>Export CSV</span></button></div></div>
      ...
      {[...new Set(visibleCart.map((item) => item.store))].map((store) => <div className="grocery-group" key={store}><h3>{store}<span>{money(visibleCart.filter((item) => item.store === store).reduce((sum, item) => sum + item.cost, 0))}</span></h3>{visibleCart.filter((item) => item.store === store).map((item) => <div key={item.product.id} className={`grocery-row ${checked.includes(item.product.id) ? "checked" : ""}`}><input aria-label={`Got ${item.product.name}`} type="checkbox" checked={checked.includes(item.product.id)} onChange={() => toggleChecked(item.product.id)} /><div><b>{item.product.name}</b><small>{item.packs===0?"Covered by pantry · ":""}{item.packs} × {item.product.packLabel} · recipe use ≈ {Math.round(item.amount)} {item.product.unit}</small><a target="_blank" rel="noreferrer" href={item.product.source}>{item.product.title} <ExternalLink size={12} /></a></div><strong>{money(item.cost)}</strong></div>)}</div>)}
...
export function RecipeModal({ meal, products, stores = [], close }: { meal: Meal; products: Product[]; stores?: string[]; close: () => void }) {
  ...
  const n = nutrition(meal.recipe, products, meal.scale * portion, stores);
  return <Dialog open onOpenChange={(open) => { if (!open) close(); }}><DialogContent className="recipe-dialog" onCloseAutoFocus={(event) => { event.preventDefault(); returnFocus?.focus(); }}>
    ...
    <DialogDescription>Preview multiples of your planned portion. This preview does not change your saved plan or shopping list.</DialogDescription>
    ...
    <h3>What you’ll need</h3><ul>{meal.recipe.ingredients.map(([id, grams]) => { const product = productFor(id, products, stores); return <li key={id}><ProductImage src={product?.image} alt={product?.title || id} /><span>{product?.name || id}</span><b>{Math.round(grams * meal.scale * portion)} {product?.unit || "g"}</b></li>; })}</ul>
    <h3>Let’s get cooking</h3><ol>{meal.recipe.steps.map((step) => <li key={step}>{step}</li>)}</ol>
    <p className="muted-small">Nutrition uses generic food estimates and may vary by brand. Product photos show ingredients, not the finished dish. Follow package instructions.</p>
