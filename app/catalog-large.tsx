...
export function Catalog({products, config, priceStatus}: {products: Product[]; config: Config; priceStatus: string}) {
  ...
  return <>
    ...
    {results ? <><button className="back-button" onClick={() => resetSearch('')}>Back to nutrition catalog</button><div className="live-grid">{results.map((product) => {const matchingOffer = product.offers.filter((item) => config.stores.includes(item.store)).sort((a, b) => a.price - b.price)[0]; return <a className="content-card live-product" href={product.url} key={product.id} target="_blank" rel="noreferrer"><h3>{product.title}</h3><b>{matchingOffer ? money(matchingOffer.price) : 'No offer in selected stores'}</b><small>{matchingOffer?.store} · Nutrition not verified</small><span>View on Arzan <ExternalLink size={14}/></span></a>;})}</div>{!results.length && <div className="empty">No products found. Try a Russian product name.</div>}</> : <>
      <div className="catalog-count"><b>{filtered.length.toLocaleString()} nutrition-mapped products</b><span>{filtered.length ? `Showing ${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, filtered.length)}` : 'No matches'}</span></div>
      <div className="content-card table-wrap"><table><thead><tr><th>Ingredient</th><th>Calories</th><th>Protein</th><th>Carbs</th><th>Fat</th><th>Pack from</th></tr></thead><tbody>{visibleProducts.map((product) => {const matchingOffer = offer(product, config.stores); return <tr key={product.id}><td><b>{product.name}</b><small>{product.category} · {product.packLabel}</small></td><td>{product.kcal} kcal</td><td>{product.p} g</td><td>{product.c} g</td><td>{product.f} g</td><td><a href={product.source} target="_blank" rel="noreferrer">{matchingOffer ? money(matchingOffer.price) : 'Unavailable'}<ExternalLink size={12}/></a><small>{matchingOffer?.store}</small></td></tr>;})}</tbody></table>{!filtered.length && <div className="empty">No matching ingredient in the catalog. Search live on Arzan for more products.</div>}</div>
